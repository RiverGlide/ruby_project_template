...
task :you_need_the_right_ruby_version => :first_you_need_versionary do
  ...
  with_guidance = "
    Oops! You are using ruby #{RUBY_VERSION}. 
    We really want you to use ruby #{minimum_ruby_version} or higher
    Suggestion:
      We like using rvm to manage our ruby versions.
      That way, you can just say  `rvm use #{minimum_ruby_version}` and hey presto, 
      you're on version #{minimum_ruby_version}
